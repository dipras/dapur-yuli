...
@section('header_title', 'EDIT STOK')

@section('subcontent')
<form action="{{ route('product.update', $product) }}" method="POST" enctype="multipart/form-data" class="px-4 py-6 space-y-6" x-data="{ imageUrl: '{{ $product->image }}', previewImage(event) { const file = event.target.files[0]; if (file) { this.imageUrl = URL.createObjectURL(file); } } }">
    @csrf
    @method('PUT')
