...
@section('subcontent')
<form action="{{ route('users.update', $user) }}" method="POST" enctype="multipart/form-data" class="px-4 py-6 space-y-6" x-data="{ avatarUrl: '{{ $user->avatar }}', previewAvatar(event) { const file = event.target.files[0]; if (file) { this.avatarUrl = URL.createObjectURL(file); } } }">
    @csrf
    @method('PUT')
